...
| R-06 | P0 | Storage | Límites de archivo (8MB, MIME) solo se validan en cliente (`validateImageFile`); no confirmado `file_size_limit`/`allowed_mime_types` a nivel de bucket. Riesgo de abuso/DoS. | `src/services/storageService.ts`, bucket `property-images` | open | — | — |
