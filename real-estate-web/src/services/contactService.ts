...
export const contactService = {
  ...
  contactViaWhatsApp(contact: ContactInfo, message?: string): ContactResult {
    if (!contact.whatsapp)
      return { success: false, method: ContactMethod.WHATSAPP, error: 'WhatsApp no disponible' }
  ...
  contactViaPhone(contact: ContactInfo): ContactResult {
    if (!contact.phone)
      return { success: false, method: ContactMethod.PHONE, error: 'Teléfono no disponible' }
  ...
  contactViaEmail(contact: ContactInfo, subject?: string, body?: string): ContactResult {
    if (!contact.email)
      return { success: false, method: ContactMethod.EMAIL, error: 'Email no disponible' }
  ...
  contactProperty(
    property: Property,
    options?: { message?: string; preferredMethod?: ContactMethod }
  ): ContactResult {
    ...
    switch (method) {
      case ContactMethod.WHATSAPP:
        if (contact.whatsapp)
          return contactService.contactViaWhatsApp(contact, options?.message ?? defaultMsg)
      ...
      case ContactMethod.EMAIL:
        if (contact.email)
          return contactService.contactViaEmail(
            contact,
